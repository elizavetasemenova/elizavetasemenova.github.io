...
\subsection{Poisson count distribution}
...
\subsection{Negative Binomial count distribution}
...
\begin{equation}
\text{P}_\text{nb}^{\lambda, \phi} \left[ N(D)=n \right] = \binom{n + \phi -1}{n} \left(\frac{\lambda}{\lambda+\phi}\right)^n \left(\frac{\phi}{\lambda+\phi}\right)^{\phi},
\end{equation}
...
\subsection{Discretization of the Gaussian Process}
